...
template<typename F,typename I,typename Matrix,typename Vector>
void assembleV_2D_t(F t,I N,I Nlb,I basis_type_test,I p,I q,const Matrix &P,const Matrix &T,const Matrix &Pb,const Matrix &Tb,Vector &v,F (*f)(F,F,F),I Gpn = 4)
{
	v = 0*v;
	for(I n = 1;n != N+1;++n){
	// vertices = P(:,T(:,n))
		...
		for(I beta = 1;beta != Nlb + 1;++beta){
			F temp=0;
			temp = Gauss_quadratrue_V_2D_t<F,I,Matrix,Vector>(t,vertices,basis_type_test,beta,Gpn,p,q,f);
			I tb = Tb(beta-1,n-1);
			v(tb-1) += temp; 
		}
	}	
