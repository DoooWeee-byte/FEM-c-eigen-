...
template<typename F,typename I,typename Matrix,typename Vector,typename SpMat>
void assembleFE_coe_pow2_2D(I der_x_coe,I der_y_coe,I basis_type_coe,Vector &solution_vec,I N,I Nlbtest,I Nlbtrial,I basis_type_test,I basis_type_trial,const Matrix &P,const Matrix &T,const Matrix &Tb_test,const Matrix &Tb_trial,SpMat &A,F (*c)(F,F),I r,I s,I p,I q,I Gpn = 4)
{
    typedef Eigen::Triplet<double> Tk;
    std::vector<Tk> tripletList;
    ...
    for(I n = 1; n != N+1; ++n) {
        ...
        for(I i=0; i < solution_vec.size();++i){
            solution_vec[i] = pow(solution_vec[i],2);
        }
        /****取出局部解向量*/
        I row = Tb_trial.rows();
        Vector uh_local_vec(row);
