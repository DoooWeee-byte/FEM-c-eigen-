...
void treat_boundary_Dirichlet_2d(Matrix &boundarynodes,Matrix &A,Matrix &Pb,Vector &b,F (*g)(F,F,F),F t)
{
    for(I k = 0; k != boundarynodes.shape[1]; ++k) {
        if(boundarynodes[0][k] == -1) {
            I i = boundarynodes[1][k];
            for(I j = 0; j != A.shape[1]; ++j) {
                A[i-1][j] = 0;
            }
            A[i-1][i-1] = 1;
            b[i-1] = g(Pb[0][i-1],Pb[1][i-1],t);
        }
    }
}
...
template<typename F,typename I,typename Matrix,typename Vector>
void treat_boundary_Robin_2d(Matrix &A1,Vector &b1,Matrix &boundaryedges,Matrix &T,Matrix &P,Matrix &Tb_trial,Matrix &Tb_test,I basis_type_trial,I basis_type_test,I der_x_trial,I der_y_trial,I der_x_test,I der_y_test,I nlb_trial,I nlb_test,F (*Robinfunc)(F,F),F (*Neufunc)(F,F),I Gpn)
{
    I nbn = boundaryedges.shape[1];
        ...
        if(boundaryedges[0][k] == -3) {
			...
			for(I alfa = 1;alfa != nlb_trial +1;++alfa){
				for(I beta = 1;beta != nlb_test+1;++beta){
					...
					A1[location_i-1][location_j-1] += temp;
				}
			}
