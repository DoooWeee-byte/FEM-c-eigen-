...
double compute_Hs_error_2D(int N,Matrix &P,Matrix &T,Matrix &Tb,int basis_type,int der_x,int der_y,Vector &solution_vec,double (*analytic_solution)(double,double,double),double t,int Gpn){
	double error = 0;
	for(int n = 1;n != N+1;++n){
//取得端点坐标放到vertices中
...
//取出Tb的第n列的值就是局部解向量在全局解向量中的位置
		...
		double int_value = Gauss_int_error_2D<double,int,Matrix,Vector>(vertices,basis_type,der_x,der_y,uh_local_vec,analytic_solution,t,Gpn);
		error += int_value;
	}
	error = sqrt(error);
	return error;
