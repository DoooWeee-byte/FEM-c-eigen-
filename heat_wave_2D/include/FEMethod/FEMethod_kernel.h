...
public:
    ...
    static F compute_Hs_error_2D(I N,Matrix &P,Matrix &T,Matrix &Tb,I basis_type,I der_x,I der_y,Vector &solution_vec,F (*analytic_solution)(F,F,F),F t,I Gpn=4) {
        return FEMethod::compute_Hs_error_2D<Matrix,Vector>(N,P,T,Tb,basis_type,der_x,der_y,solution_vec,analytic_solution,t,Gpn);
    }
    ...
    static void assembleA_2D(I N,I Nlbtest,I Nlbtrial,I basis_type_trial,I basis_type_test,const Matrix &P,const Matrix &T,const Matrix &Tb_trial,const Matrix &Tb_test,Matrix &A,F (*c)(F,F),I r,I s,I p,I q,I Gpn = 4)
    {
        return FEMethod::assembleA_2D<F,I,Matrix,Vector>(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb_trial,Tb_test,A,c,r,s,p,q,Gpn);
    }
    static void assembleV_2D(I N,I Nlb,I basis_type_test,I p,I q,const Matrix &P,const Matrix &T,const Matrix &Pb,const Matrix &Tb,Vector &v,F (*f)(F,F,F),F t,I Gpn = 4)
    {
        return FEMethod::assembleV_2D<F,I,Matrix,Vector>(N,Nlb,basis_type_test,p,q,P,T,Pb,Tb,v,f,t,Gpn);
    }
	...
	static void treat_boundary_Dirichlet_2d(Matrix &boundarynodes,Matrix &A,Matrix &Pb,Vector &b,F (*g)(F,F,F), F t)
	{
		return FEMethod::treat_boundary_Dirichlet_2d<F,I,Matrix,Vector>(boundarynodes,A,Pb,b,g,t);
	}
	...
	static void ODE_HEAT_Solver(F Time,F delta_t,F theta,Matrix &A,Vector &x,I N,I Nlb,I basis_type_test,I p,I q,F (*f)(F,F,F),F (*g)(F,F,F),F (*u0)(F,F),Matrix &M,Matrix &P,Matrix &T,Matrix &Pb,Matrix &Tb,Matrix &boundarynodes){
		return FEMethod::ODE_HEAT_Solver<F,I,Vector,Matrix>(Time,delta_t,theta,A,x,N,Nlb,basis_type_test,p,q,f,g,u0,M,P,T,Pb,Tb,boundarynodes);
	}
	static void ODE_WAVE_Solver(F Time,F delta_t,F theta,Matrix &A,Vector &x,I N,I Nlb,I basis_type_test,I p,I q,F (*f)(F,F,F),F (*g)(F,F,F),F (*u0)(F,F),F (*u00)(F,F),Matrix &M,Matrix &P,Matrix &T,Matrix &Pb,Matrix &Tb,Matrix &boundarynodes){
		return FEMethod::ODE_WAVE_Solver<F,I,Vector,Matrix>(Time,delta_t,theta,A,x,N,Nlb,basis_type_test,p,q,f,g,u0,u00,M,P,T,Pb,Tb,boundarynodes);
	}
};	
