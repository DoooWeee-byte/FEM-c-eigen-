...
int main() {
    ...
    int nlb = 3;
	int N = 2*N1*N2;
    SpMat A((N1+1)*(N2+1),(N1+1)*(N2+1)),A1(A.rows(),A.cols()),A2(A.rows(),A.cols()),A3(A.rows(),A.cols()),A4(A.rows(),A.cols()),A5(A.rows(),A.cols()),A6(A.rows(),A.cols()),A7(A.rows(),A.cols());
    ...
    //组装A矩阵和V向量
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A1,rc,1,0,1,0);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A2,rc,0,1,1,0);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A3,rc,1,0,0,1);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A4,rc,0,1,0,1);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A5,rc,1,0,0,0);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A6,rc,0,1,0,0);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A7,c,0,0,0,0);
    FEKernel::assembleV_2D(N,Nlbtest,basis_type_test,0,0,P,T,Pb,Tb,V,f);
	A = 10*A1-A2-A3+2*A4+A5+A6+A7;
