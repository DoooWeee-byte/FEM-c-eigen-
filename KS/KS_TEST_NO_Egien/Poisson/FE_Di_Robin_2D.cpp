...
double Robinfunc(double x,double y){
	return c(x,y)*r(x,y); 
}
...
int main() {
    ...
    /* FEKernel::generate_PbTb_2D(Pb,Tb,N1,N2,basis_type_trial,right,top,bottom,left);//Tb_trial,Tb_test都是Tb */

    //生成边界矩阵，边界边，边界点
    /* Matrix boundaryedges(4,2*N1+2*N2); */
    /* FEKernel::generate_boundaryedges_DR_2D(boundaryedges,N1,N2); */
    /* Matrix boundarynodes(2,2*(N1+N2)); */
    /* FEKernel::generate_boundarynodes_DR_2D(boundarynodes,N1,N2); */
    //组装A矩阵和V向量
    /* int N = 2*N1*N2; */
    /* Matrix A1((N1+1)*(N2+1),(N1+1)*(N2+1)); */
    /* Vector V((N1+1)*(N2+1)); */
    /* int Nlbtest = 3; */
    /* int Nlbtrial = 3; */
    /* FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A1,c,1,0,1,0); */
    /* FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A1,c,0,1,0,1); */
    ...
    FEKernel::generate_PbTb_2D(Pb,Tb,N1,N2,basis_type_trial,right,top,bottom,left);//Tb_test=Tb_trial=Tb
    //生成边界矩阵，边界边，边界点
    Matrix boundaryedges(4,2*N1+2*N2);
    FEKernel::generate_boundaryedges_DR_2D(boundaryedges,N1,N2);
    Matrix boundarynodes(2,nbn);
    FEKernel::generate_boundarynodes_DR_2D(boundarynodes,N1_basis,N2_basis);
    //组装A矩阵和V向量
    Matrix A1(Nb,Nb);
    Vector V(Nb);
    FEKernel::assembleA_2D(N,Nlbtest,Nlbtrial,basis_type_trial,basis_type_test,P,T,Tb,Tb,A1,c,1,0,1,0);
	...
	//边界条件处理：先处理Robin条件再处理Dirichlet条件*****************************************************
    Vector b1(V.size);
	Matrix A2(A1.shape[0],A1.shape[1]);
	int der_y_trial = 0,der_x_trial=0,der_x_test=0,der_y_test=0;
	FEKernel::treat_boundary_Robin_2d(A2,b1,boundaryedges,T,P,Tb,Tb,basis_type_trial,basis_type_test,der_x_trial,der_y_trial,der_x_test,der_y_test,Nlbtrial,Nlbtest,Robinfunc,Neufunc,4);
	Matrix A(A2.shape[0],A2.shape[1]);
	A = A1+A2;
    V = V + b1;
