...
void treat_boundary_Dirichlet_2d(Matrix &boundarynodes,Matrix &A,Matrix &Pb,Vector &b,F (*g1)(F,F,F),F (*g2)(F,F,F),F t)
{
    I Nb = Pb.cols();
    for(I k = 0; k != boundarynodes.cols(); ++k) {
        if(boundarynodes(0,k) == -1) {
            ...
            A(i-1,i-1) = 1;
            A(i-1+Nb,i-1+Nb) = 1;
            b(i-1) = g1(Pb(0,i-1),Pb(1,i-1),t);
            b(i-1+Nb) = g2(Pb(0,i-1),Pb(1,i-1),t);
        }
    }
}
template<typename F,typename I,typename Matrix,typename Vector,typename SpMat>
void treat_boundary_Dirichlet_2d_SpMat(Matrix &boundarynodes,SpMat &A,SpMat &A1,Matrix &Pb,Vector &b,F (*g1)(F,F,F),F (*g2)(F,F,F),F t)
{
