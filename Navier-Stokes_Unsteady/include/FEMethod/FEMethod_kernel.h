...
public:
	...
	static F compute_Max_error_2D(I N,Matrix &P,Matrix &T,Matrix &Tb,I basis_type,Vector &solution_vec,F (*analytic_solution)(F,F),I Gpn = 4){
		return FEMethod::compute_Max_error_2D<Matrix,Vector,F,I>(N,P,T,Tb,basis_type,solution_vec,analytic_solution,Gpn);	
	}
    ...
    static void ODE_Unsteady_NaStokes(F Tmax,F Tmin,I N_t,I N,I Nlb,I Nb,I basis_type_test,F theta,SpMat &M,SpMat &A,Matrix &P,Matrix &T,Matrix &Pb,Matrix &Tb,Matrix &boundarynodes,Vector &x,Vector &x0,F (*f1)(F,F,F),F (*f2)(F,F,F),F (*g1)(F,F,F),F (*g2)(F,F,F)){
		return FEMethod::ODE_Unsteady_NaStokes<F,I,Matrix,Vector,SpMat>(Tmax,Tmin,N_t,N,Nlb,Nb,basis_type_test,theta,M,A,P,T,Pb,Tb,boundarynodes,x,x0,f1,f2,g1,g2);
	}

    static void assembleV_2D_t(F t,I N,I Nlb,I basis_type_test,I p,I q,const Matrix &P,const Matrix &T,const Matrix &Pb,const Matrix &Tb,Vector &v,F (*f)(F,F,F),I Gpn = 4) {
        return FEMethod::assembleV_2D_t<F,I,Matrix,Vector>(t,N,Nlb,basis_type_test,p,q,P,T,Pb,Tb,v,f,Gpn);
    }
    	static void addSpMat(SpMat &A,SpMat &M,I rowstart,I colstart){
		return FEMethod::addSpMat<I, SpMat>(A,M,rowstart,colstart);
	}
};
