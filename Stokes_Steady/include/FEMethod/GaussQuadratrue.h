...
template<typename F,typename I,typename Matrix,typename Vector>
F Gauss_quadratrue_K_2D(Matrix &vertices,I basis_type_trial,I basis_type_test,I alfa,I beta,I r,I s,I p,I q,I Gpn,F (*c)(F,F),Vector uh_local_vec,I kx,I ky) {
// alfa对应trial,beta对应test
    Vector Glc(Gpn);
    Matrix Glp(2,Gpn);
    Vector Gc(Gpn);
    ...
    for(I k = 0; k != nlb; ++k ) {
        for(I i = 0; i != Gpn; ++i) {
            //高斯权重*某个系数函数*test函数的某阶导数*trial函数的某阶导数
            ret += Glc(i)*uh_local_vec(k)*local_basis_2D(Glp(0,i),Glp(1,i),vertices,basis_type_trial,k+1,kx,ky)*c(Glp(0,i),Glp(1,i))*local_basis_2D(Glp(0,i),Glp(1,i),vertices,basis_type_trial,alfa,r,s)*local_basis_2D(Glp(0,i),Glp(1,i),vertices,basis_type_test,beta,p,q);
        }
    }
    return ret;
}
